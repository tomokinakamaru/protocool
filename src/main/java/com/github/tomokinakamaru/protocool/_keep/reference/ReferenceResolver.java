...
public class ReferenceResolver extends TreeAnalyzer {
  ...
  public void enterReference(ReferenceContext ctx) {
    ...
    } else if (context.classTables.get(context.specificationContext).containsKey(name)) {
      context.references.put(ctx, context.classTables.get(context.specificationContext).get(name));
    } else if (context.importTables.get(context.specificationContext).containsKey(name)) {
      context.references.put(ctx, context.importTables.get(context.specificationContext).get(name));
    } else {
